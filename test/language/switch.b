...
union Result {
    Success { code: i32; message: i8*; }
    Error { error_code: i32; details: i8*; }
    Warning { level: i32; }
    Info {}
};
...
func testUnionSwitch() {
    ...
    let success: Result = Result::Success { code = 200, message = "OK" };
    switch (success) {
    ...
    let error: Result = Result::Error { error_code = 404, details = "Not Found" };
    switch (error) {
        case Result::Success as s:
            printf("Success: %d - %s\n", s.code, s.message);
            break;
    ...
    let warning: Result = Result::Warning { level = 2 };
    switch (warning) {
        case Result::Success as s:
            printf("Success: %d - %s\n", s.code, s.message);
            break;
...
func testNestedSwitch() {
    ...
    let task = Task { id = 42, status = Status::RUNNING, priority = Priority::HIGH };
...
func testLetExpressions() {
    ...
    let result: Result = Result::Success { code = 201, message = "Created" };
