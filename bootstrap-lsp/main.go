...
func main() {
	...
	if config.showVersion {
		fmt.Printf("bootstrap Language Server %s\n", version)
		os.Exit(0)
	}
...
func parseFlags() Config {
	...
	bootstrapPath := flag.String("bootstrap-path", "boostrap", "Path to bootstrap compiler")
...
func semaFile(s *Server, path string) {
	...
	// bootstrap with the input from the file.
	...
	if err != nil {
		log.Printf("failed to get stderr from bootstrap command: %v", err)
		return
	}

	if err := cmd.Start(); err != nil {
		log.Printf("failed to start bootstrap command: %v", err)
		return
	}
